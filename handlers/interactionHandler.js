...
module.exports = (bot) => bot.on("ready", async ()=> {
    ...
    bot.on("interactionCreate", async (interaction)=> {
        ...
        const memGuild = bot.Memory.guilds[member.guild.id];
        if(!memGuild) bot.Memory.guilds[member.guild.id] = bot.createGuild(interaction);
        const memMember = bot.Memory.guilds[member.guild.id].members[member.user.id];
        if(!memMember) bot.Memory.guilds[member.guild.id].members[member.user.id] = bot.createMember(interaction);
